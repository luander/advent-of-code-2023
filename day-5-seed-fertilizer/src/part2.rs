...
pub fn process(input: &str) -> i64 {
    ...
    while let Some(start) = seeds_line.next() {
        if let Some(range) = seeds_line.next() {
            seeds_range.push((start.parse().unwrap(), range.parse().unwrap()));
        }
    }

    let mut results = Vec::new();
    for (start, range) in seeds_range {
        results.extend(
            (start..start + range)
                .into_par_iter()
                .map(|seed| almanac.get_mapped(seed))
                .collect::<Vec<i64>>(),
        );
    }
