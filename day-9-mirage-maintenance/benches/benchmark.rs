...
use day_9_mirage_maintenance::*;

fn criterion_benchmark_part1(c: &mut Criterion) {
    let input = include_str!("../input.txt");

    let mut group = c.benchmark_group("day_9_mirage_maintenance::part1");
...
fn criterion_benchmark_part2(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("day_9_mirage_maintenance::part2");
